...
export default class History {
  private static readonly container = NeverNull(
    document.getElementById("history-container")
  );

  private static readonly axisStyle = window.getComputedStyle(
    NeverNull(document.getElementById("history-axis-style"))
  );

  private static readonly tooltipStyle = window.getComputedStyle(
    NeverNull(document.getElementById("history-tooltip-style"))
  );

  private static readonly title = NeverNull(
    document.getElementById("history-title")
  );

  private static readonly canvas = document.getElementById(
    "history-canvas"
          ...
          onClick: (event: MouseEvent, legendItem: ChartLegendLabelItem) => {
            if (legendItem.text === quantityLabel) {
              const quantityAxis = NeverUndefined(
                NeverUndefined(NeverUndefined(newChart.config.options).scales)
                  .yAxes
              )[1];
              quantityAxis.display = !quantityAxis.display;
            }
            const onClick = NeverUndefined(
              NeverUndefined(Chart.defaults.global.legend).onClick
            );
            ...
            if (
              ...
              const metaHidden = newChart.getDatasetMeta(
                legendItem.datasetIndex
              ).hidden;
              const hidden =
                (metaHidden === null
                  ? NeverUndefined(newChart.data.datasets)[
                      legendItem.datasetIndex
                    ].hidden
                  : metaHidden) || false;
          ...
          yAxes: [
            {
              id: "y-axis-gold",
              ticks: {
                callback: (value: number) => `${value / 10000}g`,
                fontColor: History.axisStyle.color || undefined,
                fontFamily: History.axisStyle.fontFamily || undefined,
                fontStyle: History.axisStyle.fontStyle || undefined,
              },
        ...
        tooltips: {
          ...
          callbacks: {
            labelColor: (tooltipItem: ChartTooltipItem, chart: Chart) => {
              const dataset = NeverUndefined(chart.data.datasets)[
            ...
            label: (tooltipItem: ChartTooltipItem, data: ChartData) => {
              const dataset = NeverUndefined(data.datasets)[
                NeverUndefined(tooltipItem.datasetIndex)
              ];
