...
export class ProfitDom {
  ...
  private static getProfitPrice(
    ...
    let price = `${formatMoney(
      auctionProfit(profit, craftsPriceType, costPriceType)
    )}`;
    if (unknown) {
      price += " - unknown";
    }
  ...
  update(profit: Profit, craftsPriceType: PriceType, costPriceType: PriceType) {
    ...
    let title = "";
    if (
      (profit.crafts && profit.crafts.auctions.length > 0) ||
    ...
    if (profit.crafts && profit.crafts.auctions.length > 0) {
      ...
      const lowestlowest = ProfitDom.getProfitPrice(
        profit,
        unknown,
        "lowest",
        "lowest"
      ...
      const lowestfarOut = ProfitDom.getProfitPrice(
        profit,
        unknown,
        "lowest",
        "farOut"
      ...
      title += `\nLowest-Far out:\t${
        lowestfarOut !== lowestlowest ? lowestfarOut : "〃"
      }`;

      const lowestfirstQuartile = ProfitDom.getProfitPrice(
        profit,
        unknown,
        "lowest",
        "firstQuartile"
      ...
      title += `\nLowest-First:    \t${
        lowestfirstQuartile !== lowestfarOut ? lowestfirstQuartile : "〃"
      }`;

      const farOutlowest = ProfitDom.getProfitPrice(
        profit,
        unknown,
        "farOut",
        "lowest"
      ...
      title += `\nFar out-Lowest:\t${
        farOutlowest !== lowestfirstQuartile ? farOutlowest : "〃"
      }`;

      const farOutfarOut = ProfitDom.getProfitPrice(
        profit,
        unknown,
        "farOut",
        "farOut"
      ...
      title += `\nFar out-Far out:\t${
        farOutfarOut !== farOutlowest ? farOutfarOut : "〃"
      }`;

      const farOutfirstQuartile = ProfitDom.getProfitPrice(
        profit,
        unknown,
        "farOut",
        "firstQuartile"
      ...
      title += `\nFar out-First:    \t${
        farOutfirstQuartile !== farOutfarOut ? farOutfirstQuartile : "〃"
      }`;
    }
    if (profit.cost.auctionSum.firstQuartile) {
      ...
      const lowest = ProfitDom.getCostPrice(
        profit.cost.auctionSum,
        unknown,
        "lowest"
      );
      title += `Cost\nLowest:\t${lowest}`;

      const farOut = ProfitDom.getCostPrice(
        profit.cost.auctionSum,
        unknown,
        "farOut"
      );
      ...
      const outlier = ProfitDom.getCostPrice(
        profit.cost.auctionSum,
        unknown,
        "outlier"
      );
      ...
      const mean = ProfitDom.getCostPrice(
        profit.cost.auctionSum,
        unknown,
        "mean"
      );
      ...
      const firstQuartile = ProfitDom.getCostPrice(
        profit.cost.auctionSum,
        unknown,
        "firstQuartile"
      );
      ...
      const secondQuartile = ProfitDom.getCostPrice(
        profit.cost.auctionSum,
        unknown,
        "secondQuartile"
      );
      ...
      const thirdQuartile = ProfitDom.getCostPrice(
        profit.cost.auctionSum,
        unknown,
        "thirdQuartile"
      );
